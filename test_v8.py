...
# 从net_train_v8.py导入必要的类和函数
from net_train_v8 import MarkerDisplacementDataset, AdvancedDisplacementPredictor, denormalize_3d
...
def load_model_and_config(model_path, config_path, num_points):
    ...
    pos_enc_scales = config.get('pos_enc_scales', [1.0, 10.0, 30.0, 100.0])
...
def evaluate_dataset(model, dataset, stats_2d, stats_disp, original_markers_3d,
    ...
    with torch.no_grad():
        for original_2d, deformed_2d, target_disp in dataset:
            # 添加batch维度并移到device
            ...
            all_pred_disp.append(pred_disp_real.cpu().numpy()[0])
            all_target_disp.append(target_disp_real.cpu().numpy()[0])
    
    # 合并所有样本
    all_pred_disp = np.concatenate(all_pred_disp, axis=0)
    all_target_disp = np.concatenate(all_target_disp, axis=0)
...
def visualize_accuracy_by_displacement(train_results, test_results,
    ...
    for a_thresh in accuracy_thresholds:
        ...
        ax.set_xlabel('位移阈值')
        ax.set_ylabel(f'准确率 (%) (误差 < {a_thresh:.2f}mm)')
        ax.set_title(f'不同位移阈值下的准确率 (误差阈值: {a_thresh:.2f}mm)')
        ax.set_xticks(x)
    ...
    for idx, (results, title) in enumerate(zip([train_results, test_results], ['Train', 'Test'])):
        ...
        ax.set_xlabel('准确率阈值')
        ax.set_ylabel('位移阈值')
        ax.set_title(f'{title} 数据集准确率热力图 (%)')
...
def visualize_2d_error_mapping(model, dataset, stats_2d, stats_disp, 
    ...
    for idx, (sample_idx, pred_disp, target_disp, input_2d_norm, error_info) in enumerate(zip(
        sample_indices, all_predicted_disp, all_target_disp, all_2d_inputs_norm, sample_errors)):
    ...
    print("样本预测统计信息")
    print("="*60)
    for idx, (sample_idx, error_info) in enumerate(zip(sample_indices, sample_errors)):
        print(f"\n样本 {sample_idx}:")
...
if __name__ == "__main__":
    ...
    # 配置文件路径
    config_path = 'model_config_advanced.pth'
    ...
    # 加载配置以获取num_points和归一化统计信息
    try:
        config = torch.load(config_path, map_location=device, weights_only=False)
        num_points = config.get('num_points', 80)
        stats_2d = config.get('stats_2d', None)
        stats_disp = config.get('stats_disp', None)
        model_type = config.get('model_type', None)
        pos_enc_scales = config.get('pos_enc_scales', None)
        print(f"从配置读取: num_points={num_points}")
        if model_type is not None:
            print(f"模型类型: {model_type}")
        if pos_enc_scales is not None:
            print(f"位置编码尺度: {pos_enc_scales}")
        if stats_2d is not None:
    ...
    for file_2d, file_3d in zip(marker_2d_files, marker_3d_files):
        try:
            basename_2d = os.path.basename(file_2d)
            basename_3d = os.path.basename(file_3d)
            if basename_2d != basename_3d:
                continue
    ...
    # 1. 在train和test数据集上计算按位移阈值的准确率
    print("\n" + "="*80)
    print("计算Train数据集上的准确率...")
    print("="*80)
    train_results, train_pred_disp, train_target_disp = evaluate_dataset(
    ...
    print("\n" + "="*80)
    print("计算Test数据集上的准确率...")
    print("="*80)
    test_results, test_pred_disp, test_target_disp = evaluate_dataset(
