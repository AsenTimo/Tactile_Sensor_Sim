...
class MarkerDisplacementPredictor(nn.Module):
    def __init__(self, num_points: int = 80):
        ...
        self.encoder_2d = nn.Sequential(
            nn.Linear(self.pos_enc_dim, 128),
            nn.BatchNorm1d(128),
            nn.GELU(),
            nn.Linear(128, 128),
            nn.BatchNorm1d(128),
    ...
    def forward(self, original_markers, deformed_markers):
        ...
        orig_flat = enc_orig.view(-1, self.pos_enc_dim)
        def_flat = enc_def.view(-1, self.pos_enc_dim)
        ...
        return outputs_flat.view(batch_size, num_points, 3)
...
if __name__ == "__main__":
    ...
    print("===== STARTING TRAINING (POS_ENC + TOP-K L1) =====")
