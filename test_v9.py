...
# 从net_train_v9.py导入必要的类和函数
from net_train_v9 import MarkerDisplacementDataset, MarkerDisplacementPredictor, denormalize_3d
...
def calculate_accuracy_by_displacement(predicted_disp, target_disp, 
    ...
    # 计算真实位移的模长
    true_displacement_magnitude = np.linalg.norm(target_disp, axis=1)  # (N,)
    
    # 计算预测误差（位移预测误差）
    pred_error = np.linalg.norm(predicted_disp - target_disp, axis=1)  # (N,)
...
def evaluate_dataset(model, dataset, stats_2d, stats_disp, original_markers_3d,
    ...
    with torch.no_grad():
        for original_2d, deformed_2d, target_disp in dataloader:
            ...
            # 模型预测位移
            pred_disp = model(original_2d, deformed_2d)  # (1, N, 3) - 归一化的
...
def visualize_accuracy_by_displacement(train_results, test_results,
    ...
    for idx, (results, title) in enumerate(zip([train_results, test_results], ['Train', 'Test'])):
        ...
        ax.set_xlabel('准确率阈值', fontsize=11)
        ax.set_ylabel('位移阈值', fontsize=11)
        ax.set_title(f'{title} 数据集准确率热力图 (%)', fontsize=12)
...
def visualize_2d_error_mapping(model, dataset, stats_2d, stats_disp, 
    ...
    with torch.no_grad():
        for original_2d, deformed_2d, target_disp in selected_samples:
             # 添加batch维度并移到device
            original_2d = original_2d.unsqueeze(0).to(device)  # (1, N, 2)
            deformed_2d = deformed_2d.unsqueeze(0).to(device)  # (1, N, 2)
            target_disp = target_disp.unsqueeze(0).to(device)  # (1, N, 3)
            ...
            all_predicted_disp.append(pred_disp_real.cpu().numpy()[0])  # (N, 3)
            all_target_disp.append(target_disp_real.cpu().numpy()[0])  # (N, 3)
            # 保存归一化的2D输入（用于可视化）
            all_2d_inputs_norm.append(deformed_2d.cpu().numpy()[0])  # (N, 2)
...
if __name__ == "__main__":
    ...
    if stats_2d is None or stats_disp is None:
        ...
        print("\n注意: net_train_v9.py使用数据归一化，预测结果需要反归一化")
    ...
    train_dataset = ConcatDataset([datasets[i] for i in train_indices])
    test_dataset = ConcatDataset([datasets[i] for i in test_indices])
